...
object MovieLensALSMain {
  def main(args: Array[String]) {
    ...
    if (args.length != 2) {
      println("Usage: /path/to/spark/bin/spark-submit --driver-memory 2g " +
        "--class dataguru.course6.MovieLensALS " +
        "target/scala-*/movielens-als-ssembly-*.jar movieLensHomeDir personalRatingsFile")
      sys.exit(1)
    }

    //设置运行环境
    val conf = new SparkConf().setAppName("MovieLensALSMain")
